...
{
	HRESULT hr = S_OK;
	sea_normal_map = std::make_unique<Texture>();
	//sea_normal_map->Load(L"Assets/Sea/nSea.png");
	//sea_normal_map->Load(L"Assets/Sea/waterB.png");
	sea_normal_map->Load("Data\\Sprite\\Sea\\pool_N.jpg");
	Create(device, "Shader\\ReflectSea_vs.cso", "Shader\\ReflectSea_ps.cso", false);
...
void ReflectSea::Draw(ID3D11DeviceContext* context, const Model* model)
{
	...
	for (const ModelResource::Mesh& mesh : resource->GetMeshes())
	{
		...
		if (mesh.nodeIndices.size() > 0)
		{
			...
			{
				DirectX::XMMATRIX world_transform = DirectX::XMLoadFloat4x4(&nodes.at(mesh.nodeIndices.at(i)).world_transform);
				DirectX::XMMATRIX offset_transform = DirectX::XMLoadFloat4x4(&mesh.offsetTransforms.at(i));
...
void ReflectSea::End(ID3D11DeviceContext* context)
{
	InActivate(context);
}
